...
import Member from "../models/Advisors.js";
...
router.get("/byYear", async (req, res) => {
   try {
      ...
      res.status(500).json({
         message: "Error fetching committee members by year",
         error,
      });
   }
});
...
router.post("/", upload.single("image"), async (req, res) => {
   try {
      const { name, role, email, phone, skills, socials } =
         req.body;
...
router.put("/:id", upload.single("image"), async (req, res) => {
   try {
      ...
      existingMember.skills = skills || existingMember.skills;
      // existingMember.validityDate = validityDate || existingMember.validityDate;
      // existingMember.isValid =
      //    isValid !== undefined
      //       ? isValid === "true" || isValid === true
      //       : existingMember.isValid;
      console.log("Received socials:", socials); // Debug log
